...
{- | Om een focus list om te zetten naar een normale list moet je de tweede lijst van de focus list omdraaien
 en aan de eerste lijst van de focus list plakken. Dit doe je met behulp van de reverse (built in) method. 
| -}
...
{- | Om van een een normale list een focus list te maken, maken we gebruik van de functie FocusList die bovenaan gemaakt is,
die maakt van twee lijst een forward en een backward (twee lijsten). Hier wordt dus FocusList list [] aangeroepen en wordt "list" geconvert 
in de forward van de focuslist en zo heb je een focuslist (maakt niet uit dat de backward leeg is).
| -}  
...
{- | Hij pakt uit de backward het eerste item, die plakt hij vervolgens voor de forward "(f:fw)" en zo is de nieuwe forward voor de
focuslist gemaakt.
| -}
...
{- | Hij pakt uit de forward het eerste item, die plakt hij vervolgens tegen de backward aan,
 let op, hij zet hem ervoor.
| -}
...
{- | Deze functie brengt de focus van een focuslist helemaal naar links. Dat doet hij door eerst een basecase 
te maken die kijkt of de tweede lijst leeg is. Is dat niet het geval, dan gaat hij op een recursieve manier
de focus een plaats naar links opschuiven (recursief omdat hij dit doet totdat hij de basecase bereikt).
Dit gebeurd met behulp van goLeft.| -}
...
{- | Deze functie brengt de focus van een focuslist helemaal naar rechts. Dat doet hij door eerst een basecase 
te maken die kijkt of er nog maar 1 item in de eerste lijst zit. Is dat niet het geval, dan gaat hij op een
recursieve manier de focus een plaats naar rechts opschuiven (recursief omdat hij dit doet totdat hij de 
basecase bereikt). Dit doet hij door het eerste item uit de forward te pakken, die plakt hij vervolgens
voor de backward.| -}
...
{- | Deze functie schuift de focus van een focus list helemaal naar rechts op, indien er niks meer rechts staat van de focus, wordt er geen 
lege lijst teruggegeven (wat wel bij mostRight gebeurd), maar mempty
|-}
...
{- | Deze functie loopt een focus list door. Dit gebeurd m.b.v. 
de map methode. Map zorgt ervoor dat een bepaalde  functie (f) op elk item van de list  wordt toegepast. 
-}
...
{- | Deze functie maakt van twee focuslists, één focuslist. Dit gebeurd door van de twee focuslist,
 paarsgewijs, een combinatie te nemen. Dit gebeurd met de zipwith methode. 
 | -}
...
{- | Deze functie geeft de eerste waarde van een lijst terug, indien de lijst leeg is wordt een
 defaultwaarde teruggegeven.
-}
